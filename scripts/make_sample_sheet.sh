...
# The pattern to match (*_R1.fastq.gz) may need to change for your file names
 for r1 in fastq_files/*_R1.fastq.gz; do
     r2="${r1/_R1/_R2}"
     sample=$(basename "$r1" | sed 's/_R1.fastq.gz//')
     long_sample=$(basename "$r1" | sed 's/_R1.fastq.gz//')
